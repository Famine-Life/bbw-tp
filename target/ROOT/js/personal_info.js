$(function() {
    console.log("ersonal_info.js !!!")
        $('.update_button').click(function () {
        console.log("update_button 按钮点击！")
        var token = $('.token').val();
...
//                        修改，修改类名或id，直接获取类名就可以
                var sid = $("#sid_span").val();
                console.log(sid);
                var value =$(this).siblings(".first_info").children("input").val();
                var arr = "sid="+sid+"&"+ which_update + "=" + value + "&" + "token=" + token+"" ;
                console.log("提交的更改信息：",arr);
                $.ajax({
                    url:$("#update_user").attr('action'),
                    //url:'/BBW/certification.do',
                    type:'post',
                    dataType:'JSON',
                    data:arr,
                    success:function (data) {
                        var result = data.result;
                        console.log("更改的返回：",data)
                        if (result === 0){
                            alert('更新失败，请检测信息格式');
                        } else if (result === 1){
                            // $(this).text(value);
